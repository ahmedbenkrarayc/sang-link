...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
  <div class="text-center mb-10">
    <h1 class="text-3xl font-bold text-gray-900">Edit Receiver</h1>
    <p class="mt-3 text-lg text-gray-600">Update receiver information</p>
  </div>
  ...
  <div class="bg-white shadow rounded-lg overflow-hidden">
    <div class="px-6 py-5 border-b border-gray-200">
      ...
      <p class="mt-1 text-sm text-gray-500">Update the receiver's information.</p>
    </div>
    <form id="receiverForm" method="POST" class="px-6 py-6">
      <!-- Add hidden field for receiver ID -->
      ...
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
        ...
        <div>
          ...
          <input type="text" name="cin" id="cin"
                 value="${receiver.cin}"
                 readonly
                 class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 bg-gray-100 text-gray-500 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">
          <p class="mt-1 text-xs text-gray-500">CIN cannot be changed</p>
        </div>
        ...
        <div>
          ...
          <select id="gender" name="gender" required
            ...
            <c:forEach var="gender" items="${genders}">
              <option value="${gender}"
                      <c:if test="${receiver.gender == gender}">selected</c:if>>
                  ${gender}
              </option>
            </c:forEach>
          </select>
        ...
        <div>
          ...
          <select id="need" name="need" required
            ...
            <c:forEach var="need" items="${needs}">
              <option value="${need}" <c:if test="${receiver.need == need}">selected</c:if>>
                  ${need}
              </option>
            </c:forEach>
          </select>
          <p class="mt-1 text-xs text-gray-500">Need level cannot be changed</p>
        </div>
      </div>
      ...
      <div class="mt-8 flex justify-end space-x-3">
        ...
        <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-primary hover:bg-secondary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
          <i class="fas fa-edit mr-2"></i> Update Receiver
        </button>
      </div>
    </form>
  </div>
