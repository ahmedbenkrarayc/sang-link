...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
  <div class="text-center mb-10">
    <h1 class="text-3xl font-bold text-gray-900">Edit Donor</h1>
    <p class="mt-3 text-lg text-gray-600">Update donor information</p>
  </div>
  ...
  <div class="bg-white shadow rounded-lg overflow-hidden">
    <div class="px-6 py-5 border-b border-gray-200">
      ...
      <p class="mt-1 text-sm text-gray-500">Update the donor's information.</p>
    </div>
    <form id="donorForm" method="POST" class="px-6 py-6">
        ...
        <div>
          ...
          <select id="gender" name="gender" required
            ...
            <c:forEach var="gender" items="${genders}">
              <option value="${gender}"
                      <c:if test="${donor.gender == gender}">selected</c:if>>
                  ${gender}
              </option>
            </c:forEach>
          </select>
      ...
      <div class="mt-8 flex justify-end space-x-3">
        ...
        <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-primary hover:bg-secondary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
          <i class="fas fa-edit mr-2"></i> Update Donor
        </button>
      </div>
    </form>
  </div>
